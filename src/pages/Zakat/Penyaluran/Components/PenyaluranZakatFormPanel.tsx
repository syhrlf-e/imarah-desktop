...
import { usePenyaluranMutation } from "@/hooks/api/useZakat";
...
interface Props {
    isOpen: boolean;
    onClose: () => void;
    mustahiqs: MustahiqShort[];
}

export default function PenyaluranZakatFormPanel({ isOpen, onClose, mustahiqs }: Props) {
    const isOnline = useNetwork();
    const { store } = usePenyaluranMutation();
    ...
    const [data, setDataForm] = useState({
        ...
        payment_channel_id: "1", // Default to 'tunai'
        notes: "",
    });
                ...
                <>
                    ...
                    <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.3, ease: "easeOut" }} className="fixed top-12 right-0 bottom-4 bg-white w-140 shadow-2xl flex flex-col z-50 rounded-tl-2xl rounded-bl-2xl border-l border-slate-200">
                        <div className="px-5 sm:px-6 py-4 border-b border-slate-100 flex items-center gap-4 shrink-0 bg-white z-10 rounded-tl-2xl">
                            ...
                            <h3 className="text-lg font-bold text-slate-900">Catat Penyaluran Zakat</h3>
                        </div>

                        <div className="flex-1 min-h-0 p-5 overflow-y-auto bg-white sm:p-6 pb-safe">
                            <form id="zakat-distribution-form" onSubmit={handleSubmit} className="space-y-6">
                                <div>
                                    <InputLabel htmlFor="mustahiq_profile_id" value="Penerima (Mustahiq) *" />
                                    <div className="mt-1">
                                        <CustomSelect
                                            value={data.mustahiq_profile_id}
                                            onChange={(val) => setData("mustahiq_profile_id", val)}
                                            options={[
                                                { value: "", label: "-- Pilih Mustahiq --" },
                                                ...mustahiqs.map(m => ({ value: m.id, label: `${m.name} (${m.ashnaf})` }))
                                            ]}
                                        />
                                    </div>
                                    <InputError message={errors.mustahiq_profile_id} className="mt-2" />
                                </div>

                                <div>
                                    <InputLabel value="Jenis Zakat yang Disalurkan *" />
                                ...
                                <div>
                                    <InputLabel value="Jumlah Penyaluran (Nominal) *" />
                                    <TextInput type="number" value={data.amount} onChange={(e) => setData("amount", e.target.value)} className="w-full mt-1" placeholder="Rp 0" />
                                    <InputError message={errors.amount} className="mt-2" />
                                </div>

                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <InputLabel value="Tanggal Salur *" />
                                        <TextInput type="date" value={data.effective_date} onChange={(e) => setData("effective_date", e.target.value)} className="w-full mt-1" />
                                    </div>
                                    <div>
                                        <InputLabel value="Metode Penyaluran *" />
                                        <div className="mt-1">
                                ...
                                <div>
                                    <InputLabel value="Keterangan / Notes" />
                                    <textarea value={data.notes} onChange={(e) => setData("notes", e.target.value)} className="block w-full mt-1 border-slate-200 rounded-xl text-sm focus:ring-emerald-500 focus:border-emerald-500" rows={3} placeholder="Misal: Bantuan sembako ramadhan..."></textarea>
                                </div>
                        ...
                        <div className="p-5 bg-white border-t border-slate-100 sm:p-6 shrink-0 pb-safe rounded-bl-2xl">
                            <FormActions onCancel={onClose} processing={processing} submitDisabled={!isOnline || !data.mustahiq_profile_id || !data.amount} layout="full-width" submitText="Simpan Penyaluran" formId="zakat-distribution-form" />
                        </div>
                    </motion.div>
                </>
            )}
